...
function GetDataByStartEnd() {
  ...
  $.ajax({
    ...
    url:
      DomainAPI +
      "SubProvider/GetBillsByType?&start=" +
      RangePages +
      "&end=10&type=5",
    method: "GET",
    headers: {
    ...
    .done(function (response) {
      ...
      for (var i = 0; i < response.data.length; i++) {
        ...
        if (
          ...
          var tdv =
            '<td class="ip"><span style="width: 100%;">' +
        ...
        TextHtml +=
          ...
          `</td>
                            ...
                            <td>` +
          response.data[i].customerName +
          ` </td>
                                                                <td>` +
          response.data[i].payId +
          `  </td>
                                                                <td>` +
          spittime(response.data[i].createDate) +
          `</td>
<td>` +
          addCommas(response.data[i].amount) +
          `</td>
                                                                <td>` +
 ...
 ` +
          tdv +
          `<td>` +
          spittime(response.data[i].payDate) +
          `</td>
<td>` +
          paidby +
          `</td>
...
function FilterBills1(range) {
  ...
  $.ajax({
    ...
    url:
      DomainAPI +
      "SubProvider/GetBillsByTypeDateTime?from=" +
      from +
      "&to=" +
      to +
      "&start=" +
...
<td>` +
 ...
 ` +
          ...
          `
 <td>` +
          (response.data[i].payDate == "0001-01-01T00:00:00"
...
function downloadcsvl() {
  ...
  $.ajax({
    url:
      DomainAPI +
      "SubProvider/GetCsvBillsFile?from=" +
      from +
      "&to=" +
      to +
      "&type=3&noteName=" +
...
function downloadexcel() {
  ...
  $.ajax({
    url:
      DomainAPI +
      "SubProvider/GetExcelBillsFile?from=" +
      from +
      "&to=" +
      to +
      "&type=3&noteName=" +
