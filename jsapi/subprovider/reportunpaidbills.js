...
function GetDataByStartEnd() {
    ...
    $.ajax({
        "async": true,
        "crossDomain": true,
        "url": DomainAPI + "SubProvider/GetBillsByType?&start=" + RangePages + "&end=10&type=2",
        "method": "GET",
        "headers": {
...
function downloadexcel() {
	...
	var notetype =  sessionStorage.getItem('notsas');
    $.ajax({
       url: DomainAPI + "SubProvider/GetExcelBillsFile?from=" + from + "&to=" + to + "&type=3&noteName="+ notetype,
        method: 'GET',
        headers: {
         "Authorization": Token
